...
# $ mkdir check_speedup
# $ cd check_speedup
# $ python "..\check_speedup.py"
#   -challenge_path "C:\workspace\perf5" 
#   -bench_lib_path "C:\workspace\benchmark"
#   -num_runs 3 -v
...
parser.add_argument("-challenge_path", type=str, help="Path to the root of the challenge.")
parser.add_argument("-v", help="verbose", action="store_true", default=False)

args = parser.parse_args()
numRuns = args.num_runs
gbenchPath = args.bench_lib_path
challengeRootPath = args.challenge_path
verbose = args.v
...
def buildAndRunBench(iterNumber, variant, cmakeFlags):
  ...
  try:
    ...
    os.chdir(buildPath)
    challengeAbsPath = challengeRootPath
    if not os.path.isabs(challengeAbsPath):
      challengeAbsPath = os.path.join(saveCWD, challengeRootPath)
    callWrapper("cmake -B . -G Ninja -DCMAKE_CXX_COMPILER=clang++ -DCMAKE_C_COMPILER=clang -DCMAKE_BUILD_TYPE=Release " + cmakeFlags + " -S \"" + challengeAbsPath + "\"")
    callWrapper("cmake --build . --config Release --parallel 8")
    callWrapper("cmake --build . --config Release --target validate")
    # this will save score in result.json file
    callWrapper("cmake --build . --config Release --target benchmark")
  except:
...
for i in range(0, numRuns):
  buildAndRunBench(i, "baseline", "-DCMAKE_CXX_FLAGS=\"  \"")
